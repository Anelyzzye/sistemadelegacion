...
class ExportaHistorial_comprador extends CI_Controller {
...
public function exportaPDF($criterio) { 
	...
	$resultadoregistro = $this->HistorialContrato_model->buscafecha($criterio);
        $pdf -> Cell(40,5,"Fecha Contrato",1,0);
         $pdf -> Cell(60,5,"Comprador",1,0);
         $pdf -> Cell(60,5,"Vendedor",1,0);
         $pdf -> Cell(70,5,"Dirección del terreno",1,0);  
         $pdf -> Cell(30,5,"Precio",1,1);
        foreach($resultadoregistro as $obj){
            $pdf -> Cell(40,5,$obj->fecha,1,0);
            $pdf -> Cell(60,5,$obj->comprador,1,0);
            $pdf -> Cell(60,5,$obj->vendedor,1,0);
            $pdf -> Cell(70,5,$obj->principal,1,0);
            $pdf -> Cell(30,5,$obj->precio,1,1);
	...
	*/
	ob_get_clean();
    $pdf -> Output('ReporteHistorialporfecha.pdf', 'I');   	
    }
}
?>